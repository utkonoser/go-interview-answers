...
1) **Unary** — один запрос, один ответ (как обычный RPC).

2) **Server streaming** — клиент шлёт один запрос, сервер — поток ответов.

3) **Client streaming** — клиент шлёт поток, сервер — один ответ.

4) **Bidirectional streaming** — оба потока одновременно. Streaming для больших данных, real-time, без буферизации всего в память.
...
1) **In-process**: `bufconn` — listener в памяти, без сети.

2) **grpc.NewClient** на тестовый сервер.

3) Моки интерфейсов сервисного слоя (не gRPC напрямую).

4) Интеграционные тесты с реальным сервером на случайном порту. Проверять status codes, metadata, deadline behaviour.
