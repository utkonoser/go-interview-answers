...
Политика хранения топика: вместо удаления по времени/размеру брокер оставляет **последнее значение для каждого ключа**. Старые записи с тем же ключом удаляются. Используется для changelog-топиков (состояние сущностей, конфиги, Kafka Connect offsets). Топик остаётся компактным, но содержит актуальный снимок по каждому ключу. Не подходит для событий, где нужна полная история — там retention по времени.
