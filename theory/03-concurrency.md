...
`sync.Cond` — условная переменная для синхронизации горутин по условию. Методы: `Wait()` — ждет сигнала, `Signal()` — будит одну горутину, `Broadcast()` — будит все. Используется когда горутины должны ждать изменения состояния, а не просто завершения других горутин. Требует связанный мьютекс.
